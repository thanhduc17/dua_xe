document.addEventListener("DOMContentLoaded", function () {
    ...
    let resultDisplayed = false; // Biến để theo dõi việc hiển thị kết quả
    ...
    function startGame() {
        if (!gameStarted && !resultDisplayed) {
            gameStarted = true;

            // Add the "car_runing" class to make the cars shake from the beginning
    ...
    function finishGame(winningCar, metrol) {
        gameStarted = false;
        resultDisplayed = true; // Đặt biến resultDisplayed thành true

        car1.style.transition = "none";
        car2.style.transition = "none";

        const confirmation = confirm(`Xe ${winningCar.className} chiến thắng! Nhấn OK để giữ nguyên vị trí.`);

        if (!confirmation) {
            // Nếu nhấn Cancel, di chuyển xe về vị trí ban đầu
            car1.style.left = "0";
            car2.style.left = "0";
        }
    ...
    function restartGame() {
        ...
        gameStarted = false;
        resultDisplayed = false; // Đặt lại resultDisplayed
